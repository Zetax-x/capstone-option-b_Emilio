...
housing_burden = (mx["gasto_vivienda_usd"] / mx["ingreso_mensual_usd"] * 100).mean()
...
for col in gasto_cols:
    spending[col] = (mx[col] / mx["ingreso_mensual_usd"] * 100).mean()

mean_ahorro = mx["ahorro_mensual_usd"].mean()
pct_neg = (mx["ahorro_negativo"] == "ahorro negativo").mean() * 100

mean_ia = mx["horas_herramientas_ia_semana"].mean()
mean_sat = mx["satisfaccion_financiera"].mean()
...
print("### Savings")
print()
print(f"- **Mean monthly savings (ahorro_mensual_usd):** ${mean_ahorro:,.2f}")
