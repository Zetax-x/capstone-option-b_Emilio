...
# ── 2. MISSING VALUES — analyse and handle ────────────────────────────────────
print("\n" + "=" * 60)
...
for val in df["pais"].unique():
    for country in valid_countries:
        # Compare ignoring accents/encoding by stripping non-ascii and lowercasing
...
# ── 5-7. Si/No columns — strip encoding artifacts ────────────────────────────
...
# ── 8. OCUPACION — fix mojibake for Diseñador Gráfico ────────────────────────
print("\n" + "=" * 60)
print("8. OCUPACION — fixing encoding artifacts")
print("=" * 60)
before = df["ocupacion"].value_counts(dropna=False).to_string()
df["ocupacion"] = df["ocupacion"].replace(
    {"DiseÃ±ador GrÃ¡fico": "Diseñador Grafico"}
)
# Also fix any other encoding variant of the same value
...
changes.append("ocupacion: 'DiseÃ±ador GrÃ¡fico' -> 'Diseñador Grafico'")
